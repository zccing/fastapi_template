...
| 变量 | 必填 | 说明 |
|---|---|---|
| `DATABASE_ASYNC_URL` | 是 | SQLAlchemy PostgreSQL 异步连接地址 |
| `ENVIRONMENT` | 是 | `LOCAL`、`TESTING`、`STAGING` 或 `PRODUCTION` |
| `DATABASE_POOL_SIZE` | 否 | 连接池大小，默认 `16` |
...
新增领域 ORM 模型时，必须继承 `src.models.Base`，并在 Alembic 加载
`Base.metadata` 前显式导入模型模块。模板不使用动态模块扫描；显式导入更容易发现
遗漏，也不会在生成迁移时执行无关模块。不要修改已经执行过的历史迁移；模型结构
变化应生成新的 revision，并人工检查 `upgrade()` 和 `downgrade()`。

例如，在 `alembic/env.py` 的配置初始化前加入：
...
POSTGRES_USER=app POSTGRES_DB=app \
  scripts/postgres/restore backup-2026-07-19-120000Z.dump --confirm-drop
```

恢复脚本只接受备份目录内的普通文件名，并在删除数据库前验证归档；同时兼容旧的
`.dump.gz` 文件。即便如此，生产恢复仍应先在隔离数据库演练并验证数据。
